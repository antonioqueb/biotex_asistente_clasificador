# Asistente Clasificador (`biotex_asistente_clasificador`)

Variante del **Asistente de clasificación** de `biotex_catalog` para Odoo 19. Misma sesión persistente, mismo
modal de edición, misma galería de imágenes y mismo contador de folios; cambia el punto en que se genera el folio.

| | Asistente de clasificación (`biotex_catalog`) | Asistente Clasificador (este módulo) |
|---|---|---|
...
**Catálogo → Asistente Clasificador** (también en Inventario → Control de inventario).
...
Requiere `biotex_catalog` ≥ 19.0.3.5.2 (ganchos `_target_brand` y `_workspace_draft_domain`).
